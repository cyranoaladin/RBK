...
\begin{itemize}
    \item \textbf{\RBKTerm{cac} (Coût d'Acquisition Client)} : Estimé à \textbf{250 TND / étudiant converti} (Campagnes LinkedIn/FB ciblées + SEO).
    ...
    \item \textbf{Taux de Chute (\RBKTerm{churn})} : 15\% entre chaque niveau (filtrage de qualité).
    \item \textbf{Taux de Placement} : 80\% sur le cycle du programme (\ProgramWeeks~semaines) (standard industrie pour formations sélectives).
\end{itemize}

\subsection{Structure des Coûts Directs}
\begin{itemize}
    \item \textbf{Mentorat} : Ratio 1:10. Rémunération horaire ou forfaitaire (Variable).
    \item \textbf{Infra} : Coût fixe LMS + Cloud (AWS/RPC) = 5\% du \RBKTerm{ca}.
    \item \textbf{Légal/Admin} : 5\% du \RBKTerm{ca}.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Top of \RBKTerm{funnel} (Leads)} : 1000 Inscrits Webinaire / Downloads Livre Blanc.
    \item \textbf{Middle of \RBKTerm{funnel} (Applicants)} : 200 Candidats passent le test technique Python/JS.
    \item \textbf{Bottom of \RBKTerm{funnel} (Selection)} : 50 Admissibles après entretien.
    \item \textbf{\RBKTerm{conversion_rate} (Cohorte)} : 30 Inscrits payants (ou boursiers validés).
\end{enumerate}
...
\begin{table}[H]
    ...
    % Colonne 1 en X (large) puis 3 colonnes fixes pour éviter le débordement
    \begin{tabularx}{\textwidth}{|X|>{\raggedleft\arraybackslash}p{2.7cm}|>{\raggedleft\arraybackslash}p{2.7cm}|>{\raggedleft\arraybackslash}p{2.7cm}|}
        \hline
