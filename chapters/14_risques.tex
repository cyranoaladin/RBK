\chapter[ANALYSE DES RISQUES \& MODÈLE DE RÉSILIENCE]{\faExclamationTriangle\ ANALYSE DES RISQUES \& MODÈLE DE RÉSILIENCE}
\label{chap:risques}
...
\begin{itemize}
    ...
    \item \textbf{Modèle Opérationnel Détaillé :} Voir \hyperref[chap:compliance]{Chapitre 16} pour l'application des Scénarios A (Export), B (Offshore) ou C (Portage) selon la maturité de l'apprenant.
...
\textbf{Mitigation :}
\begin{itemize}
    \item \textbf{Architecture Privacy-First :} Aucun nom, email ou IP n'est stocké sur la blockchain. Le SBT contient uniquement un \textit{Hash Cryptographique} (ex: \texttt{Keccak256(Diplome\_PDF)}).
