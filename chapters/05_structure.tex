...
\section{Architecture Cursus : 48 Semaines}

\subsubsection{Vue d'Ensemble : Phases et Objectifs}
\textbf{48 semaines au total = 44 semaines techniques + 4 semaines de professionnalisation.}
...
Pour maximiser l'accessibilité et la réussite, le cursus de 48 semaines est découpé en 3 niveaux certifiants et indépendants ("Stackable"). Ce modèle permet aux étudiants de valider des jalons intermédiaires, de réduire le risque financier, et de ne s'engager sur la suite qu'après avoir prouvé leur compétence. Chaque niveau délivre une valeur tangible immédiate : une compétence technique, une preuve vérifiable (SBT), et un accès réseau. L'étudiant peut s'arrêter après le N1 avec un profil junior employable, ou continuer pour viser l'excellence "Studio".

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=1.5cm, auto]
        % Nodes (Boîtes) - Espacement horizontal augmenté (0 -> 5 -> 10)
        \node[draw, fill=SolanaBlue!10, rounded corners, minimum width=3.5cm, minimum height=1.5cm, align=center, drop shadow] (n1) at (0,0) {\textbf{Niveau 1}\\Fondations\\(12 sem)};
        \node[draw, fill=SolanaPurple!10, rounded corners, minimum width=3.5cm, minimum height=1.5cm, align=center, drop shadow] (n2) at (5,2.5) {\textbf{Niveau 2}\\Spécialisation\\(16 sem)};
        \node[draw, fill=SolanaGreen!10, rounded corners, minimum width=3.5cm, minimum height=1.5cm, align=center, drop shadow] (n3) at (10,5) {\textbf{Niveau 3}\\Studio\\(20 sem)};
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|l|X|l|}
        ...
        \textbf{1. Fondations} & 12 Sem. & Débutant (Motivé) & CLI Rust, Audit Trail, Mini-App & SBT Fundamentals \\ \hline
        \textbf{2. Track} & 16 Sem. & Gate 1 (ou Test) & DApp Complexe, Tests E2E, CI/CD & SBT Specialist \\ \hline
        \textbf{3. Studio} & 20 Sem. & Gate 2 (ou Portfolio) & Capstone Audité, Demo Publique & SBT Graduate \\ \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l X X c}
        ...
        \midrule
        \textbf{Ph. 0} & Rust CLI (grep-like) & No panic, Zéro warning, Tests unitaires & > 80/100 \\
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[phase] (phase1) {\textbf{\color{SolanaPurple}NIVEAU 1 : FONDATIONS (12 Semaines)}\\ \footnotesize Piscine Rust (4s) + Fondations Web3 (8s). Gate: Mini-DApp \& CLI};
        
        \node[phase, below=of phase1] (phase2) {\textbf{\color{SolanaPurple}NIVEAU 2 : SPÉCIALISATION (16 Semaines)}\\ \footnotesize Track A (Solana) / B (EVM) / C (Product). Gate: Protocole Complexe};
        
        \node[phase, below=of phase2] (phase3) {\textbf{\color{SolanaPurple}NIVEAU 3 : PROFESSIONNALISATION (20 Semaines)}\\ \footnotesize Capstone Studio (16s) + Audit \& Carrière (4s). Gate: Mainnet Launch};

        \draw[arrow] (phase1) -- (phase2);
        \draw[arrow] (phase2) -- (phase3);

    \end{tikzpicture}
    \caption{Architecture Temporelle Alignée (44 Sem. Tech + 4 Sem. Carrière = 48 Semaines)}
\end{figure}
...
\subsubsection{Plan de Progression (16 Semaines)}
