...
Nous introduisons ici le cadre interopérable qui évite les silos et prépare les étudiants aux architectures \RBKTerm{cross_chain} modernes.
...
\begin{itemize}
    \item \textbf{\RBKTerm{bridge} (Lock \& Mint)}\footnote{\textbf{Bridge :} Protocole ou infrastructure permettant de transférer des actifs (Tokens) d'une blockchain à une autre.} : Verrouiller un actif sur la chaîne A pour en créer une représentation sur la chaîne B.
...
\begin{itemize}
    \item Un schéma d'architecture \RBKTerm{cross_chain} (flux des actifs).
    \item Une implémentation de transfert de message (ex: "Hello World" \RBKTerm{cross_chain}).
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % Bridge Components
        \node[component] (relayer) at (5, 2.5) {Relayer / Oracle\\(Off-chain)};
        \node[component] (guardian) at (5, -2.5) {Guardian Network\\(Consensus)};

        % Flows
        % Labels positioned along the path, sloped for readability
        \draw[link] (evm) -- node[above, sloped, font=\footnotesize, pos=0.5, yshift=2pt] {1. Emit Event} (relayer);
        \draw[link] (relayer) -- node[above, sloped, font=\footnotesize, pos=0.5, yshift=2pt] {2. Submit Proof} (svm);

        \draw[link, dashed] (evm) -- node[below, sloped, font=\footnotesize, pos=0.5, yshift=-2pt] {Verify} (guardian);
        \draw[link, dashed] (guardian) -- node[below, sloped, font=\footnotesize, pos=0.5, yshift=-2pt] {Sign VAA} (svm);

        % Legend
        \node[draw, fill=white, align=left] at (5,0) {\scriptsize \textbf{Légende :}\\ \textcolor{SolanaBlue}{$\rightarrow$} Flux de Données\\ \textcolor{BaseDark}{---} Zone de Trust};
