...
\section{Protocole de Performance Durable (Ex : Anti-Burnout)}
\label{sec:wellness}

Chez RBK, nous ne confondons pas "bien-être" et "tourisme". **La récupération fait partie de l'entraînement**, comme pour un athlète de haut niveau. 

\begin{ceoBox}{Philosophie : "Rest is a Feature, not a Bug"}
Un ingénieur fatigué écrit du code vulnérable. Notre protocole de santé mentale est une mesure de **sécurité industrielle** avant d'être une mesure humaniste.
\end{ceoBox}

\subsection{Le "Traffic Light Protocol"}
Chaque matin, l'étudiant déclare son état (Anonymisé) :
\begin{itemize}
    \item \textbf{Vert :} Prêt à pusher.
    \item \textbf{Orange :} Fatigue cognitive, passage en mode "Review Only".
    \item \textbf{Rouge :} Stop immédiat. 1/2 journée off obligatoire. Si > 2 rouges/semaine, intervention mentor.
\end{itemize}
\begin{table}[H]
