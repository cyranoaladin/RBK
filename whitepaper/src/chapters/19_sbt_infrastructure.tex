...
\begin{itemize}
    ...
    \item \textbf{Standard} : \texttt{ERC-5192} (Minimal Soulbound NFTs) sur EVM.
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=2cm, auto, font=\small]
        \node (student) [draw, circle, fill=gray!10] {Étudiant};
        \node (lms) [draw, rectangle, right of=student, node distance=3cm] {LMS (Web2)};
        \node (oracle) [draw, rectangle, right of=lms, node distance=3cm, fill=SolanaBlue!10] {Oracle RBK};
        \node (contract) [draw, rectangle, right of=oracle, node distance=3.5cm, fill=SolanaPurple!10] {SC Factory};
        \node (wallet) [draw, cloud, right of=contract, node distance=3cm] {Wallet (SBT)};

        \draw[->] (student) -- node[above] {Valide Gate} (lms);
        \draw[->] (lms) -- node[above] {Trigger} (oracle);
        \draw[->] (oracle) -- node[above] {Signe Tx} (contract);
        \draw[->] (contract) -- node[above] {Mint} (wallet);
    \end{tikzpicture}
...
Les partenaires B2B (Job Board) peuvent interroger le Smart Contract pour filtrer les candidats :
\textit{"Montre-moi tous les wallets qui détiennent le SBT 'Solana Advanced' ET le SBT 'Security Auditor'."}
Cela réduit le temps de screening de plusieurs jours à quelques millisecondes.
