...
\section{Le Standard Foundry (Pourquoi pas Hardhat ?)}
RBK a fait le choix radical de \textbf{Foundry} comme unique toolchain supportée.
\begin{itemize}
    \item \textbf{Vitesse} : Tests compilés en Rust, exécution 20x plus rapide.
    \item \textbf{Fuzzing Natif} : Tests de propriété intégrés (\texttt{vm.assume}, \texttt{vm.expectRevert}).
    \item \textbf{Scripting en Solidity} : Plus de Context Switching (JS/TS) pour les déploiements.
\end{itemize}

\section{Module "War Room" (Simulation Incidents)}
Le Module 6 immerge l'étudiant dans une simulation de crise :
\begin{enumerate}
    \item \textbf{J-1} : Déploiement d'un protocole vulnérable sur Testnet.
    \item \textbf{J-2} : Attaque "Red Team" (Reentrancy, Flash Loan) générant une perte de fonds simulée.
    \item \textbf{J-3} : "Blue Team" doit patcher, redeployer et sécuriser les fonds restants.
    \item \textbf{J-4} : Rédaction du Post-Mortem.
\end{enumerate}
