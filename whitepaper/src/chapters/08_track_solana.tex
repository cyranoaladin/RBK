...
% =========================================================
% Ajout critique : Architecture Anchor Production
\section{Architecture "Money Factory Standard" (Anchor)}
L'excellence technique commence par une structure de code irréprochable. Nous imposons le standard suivant pour tous les projets finaux :

\begin{techBox}{File Structure}
\begin{lstlisting}[language=bash]
programs/rbk-protocol/
├── src/
│   ├── instructions/    (1 file par instruction)
│   │   ├── mod.rs
│   │   ├── deposit.rs
│   │   └── withdraw.rs
│   ├── state/           (Comptes et types)
│   │   ├── vault.rs
│   │   └── user_profile.rs
│   ├── errors.rs        (Custom Errors avec msg!)
│   ├── events.rs        (Indexing hooks)
│   └── lib.rs           (Entrypoint ultra-léger)
\end{lstlisting}
\end{techBox}

\section{Sécurité Hardcore (OtterSec Checklist)}
Au-delà des tests, chaque PR doit valider cette checklist de survie :
\begin{itemize}
    \item[$\square$] \textbf{Signer Checks} : Tous les comptes sensibles (Owner, Admin) ont-ils \texttt{is\_signer=true} ?
    \item[$\square$] \textbf{Ownership Checks} : Le compte Token modifié appartient-il bien au PDA du programme ?
    \item[$\square$] \textbf{Arithmetic Overflows} : Utilisation de \texttt{checked\_add} / \texttt{checked\_sub} ou \texttt{overflow-checks = true}.
    \item[$\square$] \textbf{Account Confusion} : Validation des discriminants de type sur tous les comptes désérialisés.
\end{itemize}
